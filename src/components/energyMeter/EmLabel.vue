<template>
	<g :id="'barlabel-'+ props.item.name">
		...
		<text :x="props.xScale(item.name) as number + props.xScale.bandwidth() / 2" :y="props.yScale(item.energy) - 10"
			:font-size=" labelfontsize -2 " text-anchor="middle" :fill="subColor(item)"> 
			{{ subString(item)
			}}
		</text>
		<!-- Name-Icon -->
		<text :x="props.xScale(item.name) as number + props.xScale.bandwidth() / 2" :y="props.height - props.margin.bottom - 5"
			:font-size=" labelfontsize" text-anchor="middle" :fill="item.color" :class="item.icon.length <=2 ? 'fas' : ''"> 
...
const labelY = computed(() =>
	(props.autarchy) ? props.yScale(props.item.energy) - 25
...
function subString (item: PowerItem) : string {
  if (props.autarchy) {
			return (props.autText + ': ' + props.autarchy.toLocaleString(undefined) + " %");
		} else {
			return "";
		}
	}
