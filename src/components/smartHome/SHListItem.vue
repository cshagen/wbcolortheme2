  ...
  <WbSubwidget :titlecolor="device.color" :fullwidth=true>
    ...
    <template #buttons>
      <div class="d-flex float-right justify-content-end align-items-center" >
        <span v-for="(temp, idx)  in device.temp" class="p-0 m-0 align-items-center d-flex">  
          <span v-if="idx < device.tempConfigured" class="my-0 badge rounded-pill tempbadge mx-1">{{  formatTemp(temp)  }}
          </span>
        </span>
        <span v-if="props.device.canSwitch" :class="switchIcon" :style="switchStyle" class="fa statusbutton mr-2 ms-4" @click="statusButtonClicked"></span>
        <span v-if="props.device.canSwitch" class="badge rounded-pill modebutton mx-2" @click="modeButtonClicked">{{ deviceMode }}</span>
...
const switchStyle = computed(() => {
  ...
  switch (props.device.status) {
    case 'on': swColor = 'var(--color-switchGreen)'
      break
    case 'detection': swColor = 'var(--color-switchBlue)'
      break
    case 'timeout': swColor = 'var(--color-switchWhite)'
      break
    case 'waiting': swColor = 'var(--color-menu)'
      break
    default: swColor = 'var(--color-switchRed)'
  }
