<template>
  <div class="d-flex flex-column justify-content-center pgselector rounded" :style="selectorstyle">

    <RadioBarInput v-if="editmode == 2" class="m-2" id="pgm2" :options="graphmodes.map((v, i) => {
      return {
        text: modenames[i],
        value: v,
    ...
    <!-- Row 1 -->
    <!-- Up-Button -->
    <span type="button" v-if="editmode == 1" class="arrowButton d-flex align-self-center mb-3 mt-3"
      :class="{ disabled: !props.showLeftButton }" @click="$emit('shiftUp')">
      <i class="fa-solid fa-xl fa-chevron-circle-up"></i>
    </span>
    <!-- Row 2 -->
    <div class="d-flex align-items-center" :class="row2layout">
      ...
      <!-- Badge -->
      <span v-if="editmode < 2" type="button" class="p-2 px-3 badge rounded-pill datebadge" @click="toggleEdit">
        {{ displayDate }}
      </span>
      <DateInput v-if="editmode == 2" :model-value="graphdate" :mode="graphData.graphMode" @update:modelValue="setDate">
      </DateInput>
    ...
    <div class="d-flex align-items-center" :class="row3layout">
      ...
      <span v-if="editmode == 1" type="button" class="p-1" @click="toggleEdit">
        <span class="fa-solid fa-xl fa-gear"></span>
      </span>
      <!-- Down Button -->
      <span v-if="editmode == 1" type="button" class="arrowButton fa-solid fa-xl fa-chevron-circle-down p-1 "
