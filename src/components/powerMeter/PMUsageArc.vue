...
//  computed: {
const draw = computed(() => { // Draw the arc using d3
  ...
  const plotdata = [usageSummary.evuOut, usageSummary.charging]
    .concat (Object.values(shDevices)
  ...
  graph.selectAll("consumers")
    ...
    .attr("stroke", (d, i) => (i == arcCount ? (d.data.power > 0 ? props.scaleColor : "null") : d.data.color));
